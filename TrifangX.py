...
def get_move():
    ...
    elapsed = end - start
    print(f"Elapsed time: {elapsed:.6f} seconds")  # 6 decimal places
    if move_notation:
...
def evaluate_chunks(chunk):
    ...
    for task in chunk:
        ...
        total = end - start
        print(f"Scoring time: {scoring_time:.10f} seconds")
        print(f"Total task time: {total:.10f} seconds")
    if scoring:
        return results
    else:
        return None
...
def evaluate_white(board, from_row, from_col, to_row, to_col, good_moves, scores, piece, black_king_row, black_king_col, captured_piece, position_history):
    ...
    if piece == '0-0-0':
        ...
        print(print_piece_move(board, piece, from_row, from_col, to_row, to_col, captured_piece, 'b'))
            ...
            if not is_king_in_check(board, black_king_row, black_king_col, 'b'):
                ...
                if draw:
                    ...
                    if not checkmate and not bad_checkmate and not stalemate:
                        ...
                        print(print_piece_move(board, best_piece, best_row, best_col, target_row, target_col, captured, 'w'))
                        best_row2, best_col2, target_row2, target_col2, best_piece2, captured2, draw2, score_time = best_move2(board)
                        scoring_time += score_time
                        if best_row2 == best_col2 == target_row2 == target_col2 == best_piece2 == captured2 == '1':
                            checkmate2 = True
                        if not checkmate2:
                            ...
                            print(print_piece_move(board, best_piece2, best_row2, best_col2, target_row2, target_col2, captured2, 'b'))
                            start = time.perf_counter()
                            current_score = score(board, 'w')
                            end = time.perf_counter()
                            scoring_time += (end - start)
                        ...
                        output = print_moves('b', number_of_moves, game_moves)
                        print(output.rstrip(' '), end='')
                        next_move = print_piece_move(board, piece, from_row, from_col, to_row, to_col, '0', 'b')
                        print(' ' + str(number_of_moves+1) + '. ' + next_move + '#')
                        return next_move
                        sys.exit()
                    elif bad_checkmate:
...
def score(board, turn):
    ...
    stats = {
        ...
        "white_light_squared_bishop": False, "white_dark_squared_bishop": False,
        "black_light_squared_bishop": False, "black_dark_squared_bishop": False,
        "fake_castled": False, "fake_castled_white": False,
        "edge_up_white_king": False, "edge_down_white_king": False,
        "edge_left_white_king": False, "edge_right_white_king": False,
        "edge_up_black_king": False, "edge_down_black_king": False,
        "edge_left_black_king": False, "edge_right_black_king": False,
        "white_king_row": -1, "white_king_col": -1,
        "black_king_row": -1, "black_king_col": -1,
        "developement": False, "opening": False, "middlegame": False, "endgame": False,
    ...
    w_kr, w_kc = find_king(board, 'w')
    b_kr, b_kc = find_king(board, 'b')
    stats["white_king_row"], stats["white_king_col"] = w_kr, w_kc
    stats["black_king_row"], stats["black_king_col"] = b_kr, b_kc

    if turn == 'b' and is_king_in_check(board, b_kr, b_kc, 'b') and is_checkmate(board, 'b'):
        return -1000
    if turn == 'w' and is_king_in_check(board, w_kr, w_kc, 'w') and is_checkmate(board, 'w'):
        return 1000

    # Piece scan and stats gathering
    pieces = 0
    for row in range(8):
        for col in range(8):
            piece = board[row][col]
            ...
            match piece:
                ...
                case 'P':
                    stats["white_pawns"] += 1
                case 'b':
                    stats["black_bishops"] += 1
                    (stats["black_light_squared_bishop"]
                     if is_light_square(row, col)
                     else stats.__setitem__("black_dark_squared_bishop", True))
                case 'B':
                    stats["white_bishops"] += 1
                    (stats["white_light_squared_bishop"]
                     if is_light_square(row, col)
                     else stats.__setitem__("white_dark_squared_bishop", True))
                case 'n':
                    stats["black_knights"] += 1
                case 'N':
                    stats["white_knights"] += 1
                case 'r':
                    stats["black_rooks"] += 1
                case 'R':
                    stats["white_rooks"] += 1
                case 'q':
                    stats["black_queens"] += 1
                case 'Q':
                    stats["white_queens"] += 1
                case 'k':
                    stats["black_king_row"], stats["black_king_col"] = row, col
                    stats["edge_up_black_king"] = row == 0
                    stats["edge_down_black_king"] = row == 7
                    stats["edge_left_black_king"] = col == 0
                    stats["edge_right_black_king"] = col == 7
                    if (row, col) == (7, 6) and board[7][5] == 'r': stats["fake_castled"] = True
                    if (row, col) == (7, 2) and board[7][3] == 'r': stats["fake_castled"] = True
                    if castled or stats["fake_castled"]: score += 1
                case 'K':
                    stats["white_king_row"], stats["white_king_col"] = row, col
                    stats["edge_up_white_king"] = row == 0
                    stats["edge_down_white_king"] = row == 7
                    stats["edge_left_white_king"] = col == 0
                    stats["edge_right_white_king"] = col == 7
                    if (row, col) == (0, 6) and board[0][5] == 'r': stats["fake_castled_white"] = True
                    if (row, col) == (0, 2) and board[0][3] == 'r': stats["fake_castled_white"] = True
                    if castled_white or stats["fake_castled_white"]: score -= 1

            if (row, col) == (7, 7):
                if all(board[r][c] == '0' for r in [0, 7] for c in [1, 2, 5, 6]):
                    stats["developement"] = True
                if pieces < 15:
                    stats["endgame"] = True
                elif stats["developement"] and (castled or stats["fake_castled"]):
                    stats["middlegame"] = True
                else:
                    stats["opening"] = True
    ...
    for row in range(8):
        for col in range(8):
            piece = board[row][col]
            if piece == 'p':
                score += score_pawn(board, row, col, 'b', stats)
            elif piece == 'P':
                score += score_pawn(board, row, col, 'w', stats)
            elif piece == 'n':
                score += score_knight(board, row, col, 'b', stats)
            elif piece == 'N':
                score += score_knight(board, row, col, 'w', stats)
            elif piece == 'b':
                score += score_bishop(board, row, col, 'b', stats)
            elif piece == 'B':
                score += score_bishop(board, row, col, 'w', stats)
            elif piece == 'r':
                score += score_rook(board, row, col, 'b', stats)
            elif piece == 'R':
                score += score_rook(board, row, col, 'w', stats)
            elif piece == 'q':
                score += score_queen(board, row, col, 'b', stats)
            elif piece == 'Q':
                score += score_queen(board, row, col, 'w', stats)
            elif piece == 'k':
                score += score_king(board, row, col, 'b', stats)
            elif piece == 'K':
                score += score_king(board, row, col, 'w', stats)
        ...
        if stats["endgame"]:
            ...
            if stats["edge_up_white_king"]:
                if board[row-2][col] == 'K':
                    score -= 1.5
                score -= (7-row)/15
            if stats["edge_down_white_king"]:
                if board[row+2][col] == 'K':
                    score -= 1.5
                score -= row/15
            if stats["edge_right_white_king"]:
                if board[row][col+2] == 'K':
                    score -= 1.5
                score -= col/15
            if stats["edge_left_white_king"]:
                if board[row][col-2] == 'K':
                    score -= 1.5
                score -= (7-col)/15
        ...
        if stats["endgame"]:
            ...
            if stats["edge_up_black_king"]:
                if board[row-2][col] == 'k':
                    score -= 1.5
                score -= (7-row)/15
            if stats["edge_down_black_king"]:
                if board[row+2][col] == 'k':
                    score -= 1.5
                score -= row/15
            if stats["edge_right_black_king"]:
                if board[row][col+2] == 'k':
                    score -= 1.5
                score -= col/15
            if stats["edge_left_black_king"]:
                if board[row][col-2] == 'k':
                    score -= 1.5
                score -= (7-col)/15
        ...
        if stats["endgame"]:
            if stats["white_pieces"] == 1:
                if stats["black_rooks"] == 2:
                    if (row + 1 == stats["white_king_row"] or row -1 == stats["white_king_row"]) and not stats["rook_placed_row"] and not stats["rook_placed_col"]:
                      score += 3
                      stats["rook_placed_row"] = True
                    if (col + 1 == stats["white_king_col"] or col - 1 == stats["white_king_col"]) and not stats["rook_placed_row"] and not stats["rook_placed_col"]:
                        score += 3
                        stats["rook_placed_col"] = True
                    elif stats["rook_placed_row"]:
                        if row == stats["white_king_row"]:
                            score += 3
                    elif stats["rook_placed_col"]:
                        if col == stats["white_king_col"]:
                            score += 3
    else:
        if board[7][6] == 'k':
        ...
        if not stats["endgame"]:
            ...
            if stats["black_pieces"] == 1:
                if stats["white_rooks"] == 2:
                    if (row + 1 == stats["black_king_row"] or row -1 == stats["black_king_row"]) and not stats["rook_placed_row_white"] and not stats["rook_placed_col_white"]:
                      score -= 3
                      stats["rook_placed_row_white"] = True
                    if (col + 1 == stats["black_king_col"] or col - 1 == stats["black_king_col"]) and not stats["rook_placed_row_white"] and not stats["rook_placed_col_white"]:
                        score -= 3
                        stats["rook_placed_col_white"] = True
                    elif stats["rook_placed_row_white"]:
                        if row == stats["black_king_row"]:
                            score -= 3
                    elif stats["rook_placed_col_white"]:
                        if col == stats["black_king_col"]:
                            score -= 3
    return score
def score_knight(board, row, col, color, stats):
    score = 0
    if color == 'b':
        if (row, col) in [(4, 3), (4, 4), (3, 3), (3, 4)]:
            score += 0.6
...
def score_pawn(board, row, col, color, stats):
    ...
    opening = stats.get("opening", False)
    endgame = stats.get("endgame", False)
    black_king_row = stats.get("black_king_row", 7)
    black_king_col = stats.get("black_king_col", 4)
    white_king_row = stats.get("white_king_row", 0)
    white_king_col = stats.get("white_king_col", 4)
    ...
    if color == 'b':
        ...
        if not endgame:
            ...
            if white_king_row == 0 and white_king_col not in {4, 5}:
                    ...
                    for direction in directions:
                        if direction == 1 and row == 6 and board[row-2][col] == '0' and board[row-1][col] == '0':
                            ...
                            if good_right and good_left:
                                ...
                                #scores = evaluate_white(board, row, col, row-2, col, good_moves, scores, piece, black_king_row, black_king_col, '0', position_history)
                                ...
                                if not is_king_in_check(board, black_king_row, black_king_col, 'b'):
                                    if board[7][4] == 'k' and board[7][7] == 'r' and king_move == 0 and board[7][5] == '0' and board[7][6] == '0':
                                        ...
                                        if not is_king_in_check(board, black_king_row, black_king_col, 'b'):
                                            ...
                                            if not is_king_in_check(board, black_king_row, black_king_col, 'b'):
                                                new_board = [row[:] for row in board]
                                                key_args = (new_board, 1, 1, 1, 1, good_moves, '0-0', black_king_row, black_king_col, 1, position_history)
                                                tasks.append(key_args)
                                ...
                                if not is_king_in_check(board, black_king_row, black_king_col, 'b'):
                                    if board[7][4] == 'k' and board[7][0] == 'r' and king_move == 0 and board[7][1] == '0' and board[7][2] == '0' and board[7][3] == '0':
                                        ...
                                        if not is_king_in_check(board, black_king_row, black_king_col, 'b'):
                                            ...
                                            if not is_king_in_check(board, black_king_row, black_king_col, 'b'):
                                                new_board = [row[:] for row in board]
                                                key_args = (new_board, 1, 1, 1, 1, good_moves, '0-0-0', black_king_row, black_king_col, 1, position_history)
                                                tasks.append(key_args)
